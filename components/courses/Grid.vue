...
<div class="grid grid-cols-1 md:grid-cols-2 gap-4">
  ...
  <template v-else>
    <Motion v-for="(course, index) in filteredCourses" :key="course.id || index" as="div" :initial="{
      y: 20,
      opacity: 0,
    }" :animate="{
      y: 0,
      ...
      <v-card variant="tonal" class="p-4" :id="course.id || index">
        <v-img :src="images.find((image) => image.course_id === String(course.id))?.image" />
        ...
        <v-card-actions class="mt-4">
          <v-btn @click="openCourse(course.url)" variant="text" color="#4ade80" prepend-icon="mdi-open-in-new">Buy
            course on Udemy</v-btn>
        </v-card-actions>
      </v-card>
    </Motion>
  </template>
